####
#### No.11.1 Visualize index-dependence
...
sample_data(ps_exp2)[,"Index_name"] <- 
  sample_data(ps_exp2)[,"I5_Index_ID2"] %>%
  unlist %>% str_extract(regex("IPg...."))
...
sample_sums(ps_exp2_even) # 95323 reads
...
ps_exp2_top_melt <- ps_exp2_even %>%
  ...
  prune_taxa(top_taxa, .) %>%
  speedyseq::psmelt()
f2_1 <- ggplot(ps_exp2_top_melt, aes(x = Index_name, y = Abundance)) +
...
ps_exp3_top_melt <- ps_exp3_even_seaN %>% prune_taxa(top_taxa, .) %>%
  speedyseq::psmelt()
f4 <- ggplot(ps_exp3_top_melt, aes(x = Index_name, y = Abundance)) +
    geom_boxplot(outlier.shape = NA, outlier.size = 0) +
    geom_jitter(width = 0.2, height = 0, alpha = 0.7) +
    facet_wrap(.~ OTU, nrow = 3) +
    panel_border() +
...
ps_exp3_melt2 <- ps_exp3_even %>%
  ...
  speedyseq::psmelt()
summary(aov(glm(cbind(Abundance, sample_sums(ps_exp3_even)[1]) ~ OTU + Index_name,
                data = ps_exp3_melt2,
    family = binomial(link = "logit"))))

summary(aov(glm(cbind(Abundance, sample_sums(ps_exp3_even)[1]) ~ Index_name,
...
ps_exp1_top_melt$Index_name[is.na(ps_exp1_top_melt$Index_name)] <-
  ps_exp1_top_melt$Index_name[is.na(ps_exp1_top_melt$Index_name)] <-
  "2nd PCR indexing"
f5 <- ggplot(ps_exp1_top_melt, aes(x = Index_name, y = Abundance)) +
  geom_boxplot(outlier.shape = NA, outlier.size = 0) +
  geom_jitter(width = 0.2, height = 0, alpha = 0.7) +
  facet_wrap(.~ OTU, nrow = 3) +
  panel_border() +
  ...
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust = 0.5, size = 10)) + 
  NULL
mean_2nd_indexing <- f5$data %>% group_by(Index_name, OTU) %>%
...
ps_exp1_top_melt2 <- ps_exp1_seaN %>%
  ...
  prune_taxa(top_taxa, .) %>% 
  speedyseq::psmelt()
summary(aov(glm(cbind(Abundance, rarefied_counts1) ~ OTU + Index_name,
